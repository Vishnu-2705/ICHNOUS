  ...
  return (
    <div className="flex flex-col gap-6 bg-slate-900/60 border border-slate-800 rounded-xl p-5 sm:p-6 shadow-xl backdrop-blur-sm">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {diagnosis.verification?.verified && (
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-400/50 shadow-sm shadow-emerald-500/20 animate-pulse">
                  ✓ Verified Fix (Pass)
                </span>
              )}
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {root_cause_node_id && confidence > 0 ? (
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 flex flex-col gap-2">
            ...
            <p className="text-xs font-mono text-slate-400">
              Available telemetry shows no divergence. Evidence is insufficient to attribute failure to a specific node.
            </p>
          </div>
        )}
      ...
      {/* Closed-Loop Sandbox Verification Card */}
      {diagnosis.verification && (
        <div className="flex flex-col gap-2 bg-emerald-950/20 border border-emerald-500/30 p-4 rounded-xl">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono font-bold text-emerald-300 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
              +{Math.round((diagnosis.verification.confidence_boost || 0) * 100)}% Confidence Boost
            </span>
          </div>
          <p className="text-xs text-slate-300 font-mono">
            {diagnosis.verification.execution_output}
          </p>
        </div>
      )}
    </div>
